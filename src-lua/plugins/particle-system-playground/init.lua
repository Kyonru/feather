  ...
  for i = 1, count do
    ...
    if sys then
      ...
      if not imageVars[imageKey] then
        ...
        lines[#lines + 1] = var .. ':setFilter("nearest", "nearest")'
      end

      if asset.shaderSource ~= "" or asset.shaderPath ~= "" then
        ...
        if not shaderVars[shaderKey] then
          ...
          local loadPath = asset.shaderPath ~= "" and asset.shaderPath or asset.shaderFilename
          lines[#lines + 1] = "local " .. var .. " = LG.newShader(love.filesystem.read(" .. quote(loadPath) .. "))"
        end
      end
    end
  end
  ...
  for i = 1, self:_systemCount(name) do
    ...
    if sys then
      ...
      if asset.shaderSource ~= "" then
        local filename = sanitizeFilename(asset.shaderFilename, "shader_" .. tostring(i) .. ".glsl")
        if not seen[filename] then
          seen[filename] = true
          files[#files + 1] = { name = filename, data = asset.shaderSource, encoding = "text" }
        end
      end
    end
  end
