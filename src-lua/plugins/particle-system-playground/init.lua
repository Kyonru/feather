...
function ParticleSystemPlaygroundPlugin:_generateCode(name)
  ...
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  lines[#lines + 1] = "local function publishSystems(instance)"
  lines[#lines + 1] = "  clearSystemsAlias()"
  lines[#lines + 1] = "  if not instance then"
  lines[#lines + 1] = "    return"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  for index, emitter in ipairs(instance.systems or {}) do"
  lines[#lines + 1] = "    systems[index] = emitter"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  lines[#lines + 1] = "local function releaseInstance(instance)"
  lines[#lines + 1] = "  if not instance then"
  lines[#lines + 1] = "    return"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  for index, emitter in ipairs(instance.systems or {}) do"
  ...
  lines[#lines + 1] = "    end"
  lines[#lines + 1] = "    instance.systems[index] = nil"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  ...
  lines[#lines + 1] = ""
  lines[#lines + 1] = "  return instance"
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  lines[#lines + 1] = "local function init()"
  lines[#lines + 1] = "  release()"
  lines[#lines + 1] = "  clearSystemsAlias()"
  lines[#lines + 1] = "  local instance = createInstance()"
  lines[#lines + 1] = "  pooledInstances[#pooledInstances + 1] = instance"
  lines[#lines + 1] = "  publishSystems(instance)"
  ...
  lines[#lines + 1] = "    local emitter = instance.systems[index]"
  ...
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  lines[#lines + 1] = "local function emitTimelineStarts(instance, previousTime, nextTime)"
  lines[#lines + 1] = "  if not instance then"
  lines[#lines + 1] = "    return"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  for index, track in ipairs(timeline.tracks or {}) do"
  ...
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  lines[#lines + 1] = "local function emitTimelineStartsForAdvance(instance, previousTime, elapsed)"
  ...
  lines[#lines + 1] = "    if segment > 0 then emitTimelineStarts(instance, cursor, nextCursor) end"
  ...
  lines[#lines + 1] = "  payload = type(payload) == \"table\" and payload or {}"
  ...
  lines[#lines + 1] = "    instance.loop = timelineState.loop"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  instance.directionOffset = tonumber(payload.r) or 0"
  lines[#lines + 1] = "  instance.systemIndex = tonumber(payload.systemIndex)"
  lines[#lines + 1] = "  for index, emitter in ipairs(instance.systems or {}) do"
  ...
  lines[#lines + 1] = "    end"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  applyTimeline(instance, 0, true)"
  lines[#lines + 1] = "  emitTimelineStarts(instance, 0, 0)"
  lines[#lines + 1] = "  return instance"
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  lines[#lines + 1] = "local function recycleInstance(instance)"
  lines[#lines + 1] = "  if not instance then"
  lines[#lines + 1] = "    return"
  lines[#lines + 1] = "  end"
  ...
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  if #pooledInstances < MAX_POOLED_INSTANCES then"
  lines[#lines + 1] = "    pooledInstances[#pooledInstances + 1] = instance"
  lines[#lines + 1] = "  else"
  lines[#lines + 1] = "    releaseInstance(instance)"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  lines[#lines + 1] = "local function checkoutInstance(payload)"
  lines[#lines + 1] = "  local instance = table.remove(pooledInstances)"
  lines[#lines + 1] = "  if not instance then"
  lines[#lines + 1] = "    instance = createInstance()"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  activeInstances[#activeInstances + 1] = instance"
  lines[#lines + 1] = "  resetInstance(instance, payload)"
  lines[#lines + 1] = "  publishSystems(instance)"
  lines[#lines + 1] = "  return instance"
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  ...
  lines[#lines + 1] = "    if instance.playing then"
  ...
  lines[#lines + 1] = "        end"
  lines[#lines + 1] = "      else"
  lines[#lines + 1] = "        emitTimelineStarts(instance, previous, nextTime)"
  lines[#lines + 1] = "      end"
  lines[#lines + 1] = "      instance.time = nextTime"
  lines[#lines + 1] = "    end"
  ...
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  return checkoutInstance(payload)"
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  ...
  lines[#lines + 1] = "  for _, instance in ipairs(activeInstances) do"
  lines[#lines + 1] = "    instance.playing = false"
  lines[#lines + 1] = "    applyTimeline(instance, instance.time, false)"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  return true"
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  ...
  lines[#lines + 1] = "    if resetParticles ~= false then"
  lines[#lines + 1] = "      table.remove(activeInstances, i)"
  lines[#lines + 1] = "      recycleInstance(instance)"
  lines[#lines + 1] = "    end"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  if resetParticles ~= false then"
  lines[#lines + 1] = "    publishSystems(nil)"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  return true"
  lines[#lines + 1] = "end"
  lines[#lines + 1] = ""
  ...
  lines[#lines + 1] = "  for i = #activeInstances, 1, -1 do"
  lines[#lines + 1] = "    releaseInstance(activeInstances[i])"
  lines[#lines + 1] = "    activeInstances[i] = nil"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  for i = #pooledInstances, 1, -1 do"
  lines[#lines + 1] = "    releaseInstance(pooledInstances[i])"
  lines[#lines + 1] = "    pooledInstances[i] = nil"
  lines[#lines + 1] = "  end"
  lines[#lines + 1] = "  clearSystemsAlias()"
  lines[#lines + 1] = "  return true"
  lines[#lines + 1] = "end"
