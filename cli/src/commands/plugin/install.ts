...
import { createSpinner, printJson, printMuted, printWarning } from '../../lib/output.js';
...
export async function pluginInstallCommand(pluginIds: string | string[], opts: PluginSourceOptions): Promise<void> {
  ...
  if (resolveManaged(projectDir, installDir, opts.managed) === 'cli') {
    await configPluginsCommand({ dir: projectDir, include: uniqueIds.join(','), dryRun: opts.dryRun, json: opts.json });
    if (opts.json) return;
    for (const pluginId of uniqueIds) {
  ...
  if (!opts.remote) {
    ...
    const spinner = opts.json ? null : createSpinner(`Copying ${uniqueIds.join(', ')}…`).start();
    let result: { installed: string[]; skipped: string[] };
    try {
      result = opts.dryRun
        ? { installed: uniqueIds.filter((pluginId) => !existsSync(join(projectDir, installDir, 'plugins', pluginIdToSourceDir(pluginId)))), skipped: [] }
        : installPluginsFromLocal(uniqueIds, sourceRoot, projectDir, installDir, undefined, force);
    } catch (err) {
    ...
    if (opts.json) {
      printJson({
        projectDir,
        managed: resolveManaged(projectDir, installDir, opts.managed) ?? null,
        dryRun: opts.dryRun === true,
        installed: installed.map((id) => pluginSummaryJson(id)),
        skipped,
      });
      return;
    }
  ...
  const installedRemote: string[] = [];
  for (const pluginId of uniqueIds) {
    const installSpinner = opts.json ? null : createSpinner(`Installing ${pluginId}…`).start();
    try {
      const result = opts.dryRun
        ? { skipped: existsSync(join(projectDir, installDir, 'plugins', pluginIdToSourceDir(pluginId))) && !force }
        : await installPlugin(pluginId, entries, projectDir, branch, undefined, installDir, force);
      if (result.skipped) {
        installSpinner?.stop();
        skippedRemote.push(pluginId);
      } else {
        installSpinner?.succeed(`Installed ${pluginId}`);
        installedRemote.push(pluginId);
        if (!opts.json) warnDangerousPlugin(pluginId);
      }
    } catch (err) {
  ...
  if (opts.json) {
    printJson({
      ...
      dryRun: opts.dryRun === true,
      remote: true,
      installed: installedRemote.map((id) => pluginSummaryJson(id)),
