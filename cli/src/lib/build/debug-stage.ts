...
function mobileMainWrapper(): string {
  return [
    ...
    'end',
    '',
    'local function installIosTextRenderGuard(config)',
    '  if type(config) ~= "table" then config = {} end',
    '  if config.iosTextRendering == true or config.iosTextRenderGuard == false then return end',
    '  if not love.system or not love.system.getOS or love.system.getOS() ~= "iOS" then return end',
    '  if not love.graphics then return end',
    '  local originalPrint = love.graphics.print',
    '  local originalPrintf = love.graphics.printf',
    '  love.graphics._featherOriginalPrint = originalPrint',
    '  love.graphics._featherOriginalPrintf = originalPrintf',
    '  love.graphics.print = function(...) end',
    '  love.graphics.printf = function(...) end',
    'end',
    '',
    'FEATHER_AUTO_CONFIG = loadFeatherConfig()',
    'installIosTextRenderGuard(FEATHER_AUTO_CONFIG)',
