...
| --- | --- | --- | --- |
| `bufferSize` | `number` | `1000` | Maximum frames stored. When the buffer is full, the oldest frame is overwritten. At 60 fps this is ~16 seconds of history. Increase for longer sessions; decrease to save memory. |
...
!!! tip
    Start with `bufferSize = 1000`. If the bug takes longer than ~15 seconds to trigger after you start recording, increase it. Very large buffers (5000+) may take a moment to transfer over the WebSocket when you click Stop & Load.
