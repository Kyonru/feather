...
export const NODE_DEFS: Record<NodeType, NodeDef> = {
  ...
  SpriteTextureSample: {
    category: 'Fake 3D',
    label: 'Sprite Texture Sample',
    inputs: [
      { id: 'uv', label: 'UV', type: 'vec2', defaultValue: [0.5, 0.5], min: 0, max: 1, step: 0.01 },
      { id: 'mask', label: 'Mask', type: 'float', defaultValue: 1, min: 0, max: 1, step: 0.01 },
    ],
    outputs: [{ id: 'rgba', label: 'RGBA', type: 'vec4' }],
    emitGlsl: (i, o) => [
      `vec4 ${o.rgba} = Texel(tex, clamp(${i.uv}, vec2(0.0), vec2(1.0)));`,
      `${o.rgba}.a *= clamp(${i.mask}, 0.0, 1.0);`,
    ].join('\n'),
  },
  BillboardUV: {
    category: 'Fake 3D',
    ...
    inputs: [
      { id: 'uv', label: 'UV', type: 'vec2', defaultValue: [0.5, 0.5], min: 0, max: 1, step: 0.01 },
      { id: 'tilt', label: 'Tilt', type: 'vec2', defaultValue: [0.28, -0.18], min: -1, max: 1, step: 0.01 },
      { id: 'perspective', label: 'Perspective', type: 'float', defaultValue: 0.45, min: -2, max: 2, step: 0.01 },
      { id: 'scale', label: 'Scale', type: 'vec2', defaultValue: [1, 1], min: 0.1, max: 3, step: 0.01 },
      { id: 'pivot', label: 'Pivot', type: 'vec2', defaultValue: [0.5, 0.5], min: 0, max: 1, step: 0.01 },
    ],
    outputs: [
      { id: 'uv', label: 'UV', type: 'vec2' },
      { id: 'mask', label: 'Mask', type: 'float' },
      { id: 'depth', label: 'Depth', type: 'float' },
    ],
    emitGlsl: (i, o) => [
      `vec2 ${o.uv}_scale = max(abs(${i.scale}), vec2(0.0001));`,
      `vec2 ${o.uv}_p = (${i.uv} - ${i.pivot}) / ${o.uv}_scale;`,
      `float ${o.depth} = clamp(0.5 + dot(${o.uv}_p, ${i.tilt}), 0.0, 1.0);`,
      `float ${o.uv}_denom = max(1.0 + (${o.depth} - 0.5) * ${i.perspective}, 0.05);`,
      `vec2 ${o.uv} = ${o.uv}_p / ${o.uv}_denom + ${i.pivot};`,
      `float ${o.mask} = step(0.0, ${o.uv}.x) * step(${o.uv}.x, 1.0) * step(0.0, ${o.uv}.y) * step(${o.uv}.y, 1.0);`,
    ].join('\n'),
  },
  ParallaxUV: {
    category: 'Fake 3D',
    ...
    inputs: [
      ...
      { id: 'view', label: 'View', type: 'vec2', defaultValue: [0.25, -0.18], min: -1, max: 1, step: 0.01 },
      { id: 'strength', label: 'Strength', type: 'float', defaultValue: 0.06, min: -0.5, max: 0.5, step: 0.001 },
      { id: 'center', label: 'Center', type: 'float', defaultValue: 0.5, min: 0, max: 1, step: 0.01 },
    ],
    outputs: [
      { id: 'uv', label: 'UV', type: 'vec2' },
      { id: 'offset', label: 'Offset', type: 'vec2' },
    ...
    emitGlsl: (i, o) => [
      `vec2 ${o.offset} = ${i.view} * (${i.height} - ${i.center}) * ${i.strength};`,
      `vec2 ${o.uv} = ${i.uv} + ${o.offset};`,
    ].join('\n'),
  },
  FakeDepthShade: {
    category: 'Fake 3D',
    label: 'Fake Depth Shade',
    inputs: [
      { id: 'color', label: 'Color', type: 'vec4', defaultValue: [1, 1, 1, 1], min: 0, max: 1, step: 0.01 },
      { id: 'depth', label: 'Depth', type: 'float', defaultValue: 0.5, min: 0, max: 1, step: 0.01 },
      { id: 'mask', label: 'Mask', type: 'float', defaultValue: 1, min: 0, max: 1, step: 0.01 },
      { id: 'lightDir', label: 'Light', type: 'vec2', defaultValue: [-0.4, -0.7], min: -1, max: 1, step: 0.01 },
      { id: 'ambient', label: 'Ambient', type: 'float', defaultValue: 0.82, min: 0, max: 2, step: 0.01 },
      { id: 'lightStrength', label: 'Light Amt', type: 'float', defaultValue: 0.35, min: 0, max: 2, step: 0.01 },
      { id: 'rimStrength', label: 'Rim Amt', type: 'float', defaultValue: 0.18, min: 0, max: 2, step: 0.01 },
    ],
    outputs: [{ id: 'rgba', label: 'RGBA', type: 'vec4' }],
    emitGlsl: (i, o) => [
      `float ${o.rgba}_depth = clamp(${i.depth}, 0.0, 1.0);`,
      `vec2 ${o.rgba}_light2 = ${i.lightDir} / max(length(${i.lightDir}), 0.0001);`,
      `vec3 ${o.rgba}_normal = normalize(vec3(-dFdx(${o.rgba}_depth) * 4.0, -dFdy(${o.rgba}_depth) * 4.0, 1.0));`,
      `vec3 ${o.rgba}_light = normalize(vec3(${o.rgba}_light2, 1.0));`,
      `float ${o.rgba}_diffuse = clamp(dot(${o.rgba}_normal, ${o.rgba}_light), 0.0, 1.0);`,
      `float ${o.rgba}_rim = pow(clamp(1.0 - ${o.rgba}_normal.z, 0.0, 1.0), 2.0);`,
      `float ${o.rgba}_shade = max(${i.ambient}, 0.0) + ${o.rgba}_diffuse * ${i.lightStrength} + ${o.rgba}_rim * ${i.rimStrength};`,
      `vec4 ${o.rgba} = vec4(clamp(${i.color}.rgb * ${o.rgba}_shade, 0.0, 1.0), ${i.color}.a * clamp(${i.mask}, 0.0, 1.0));`,
    ].join('\n'),
  },
  BillboardShadow: {
    category: 'Fake 3D',
    label: 'Billboard Shadow',
    inputs: [
      { id: 'uv', label: 'UV', type: 'vec2', defaultValue: [0.5, 0.5], min: 0, max: 1, step: 0.01 },
      { id: 'mask', label: 'Mask', type: 'float', defaultValue: 1, min: 0, max: 1, step: 0.01 },
      { id: 'depth', label: 'Depth', type: 'float', defaultValue: 0.5, min: 0, max: 1, step: 0.01 },
      { id: 'offset', label: 'Offset', type: 'vec2', defaultValue: [0.06, 0.08], min: -0.5, max: 0.5, step: 0.001 },
      { id: 'softness', label: 'Soft', type: 'float', defaultValue: 0.14, min: 0.001, max: 1, step: 0.01 },
      { id: 'opacity', label: 'Opacity', type: 'float', defaultValue: 0.42, min: 0, max: 1, step: 0.01 },
      { id: 'color', label: 'Color', type: 'vec4', defaultValue: [0, 0, 0, 1], min: 0, max: 1, step: 0.01 },
    ],
    outputs: [
    ...
    emitGlsl: (i, o) => [
      `vec2 ${o.mask}_uv = ${i.uv} - ${i.offset} * (0.35 + clamp(${i.depth}, 0.0, 1.0));`,
      `vec2 ${o.mask}_box = abs(${o.mask}_uv - vec2(0.5)) - vec2(0.5);`,
      `float ${o.mask}_dist = max(${o.mask}_box.x, ${o.mask}_box.y);`,
      `float ${o.mask} = (1.0 - smoothstep(0.0, max(${i.softness}, 0.0001), ${o.mask}_dist)) * clamp(${i.mask}, 0.0, 1.0) * clamp(${i.opacity}, 0.0, 1.0);`,
      `vec4 ${o.rgba} = vec4(${i.color}.rgb, ${i.color}.a * ${o.mask});`,
    ].join('\n'),
  },
  AtlasSliceUV: {
    category: 'Fake 3D',
    label: 'Atlas Slice UV',
    inputs: [
      { id: 'uv', label: 'UV', type: 'vec2', defaultValue: [0.5, 0.5], min: 0, max: 1, step: 0.01 },
      { id: 'slice', label: 'Slice', type: 'float', defaultValue: 0, min: 0, max: 16, step: 1 },
      { id: 'columns', label: 'Columns', type: 'float', defaultValue: 4, min: 1, max: 16, step: 1 },
      { id: 'rows', label: 'Rows', type: 'float', defaultValue: 4, min: 1, max: 16, step: 1 },
    ],
    outputs: [
      { id: 'uv', label: 'UV', type: 'vec2' },
      { id: 'mask', label: 'Mask', type: 'float' },
    ...
    emitGlsl: (i, o) => [
      `float ${o.uv}_columns = max(floor(${i.columns} + 0.5), 1.0);`,
      `float ${o.uv}_rows = max(floor(${i.rows} + 0.5), 1.0);`,
      `float ${o.uv}_cell_count = max(${o.uv}_columns * ${o.uv}_rows, 1.0);`,
      `float ${o.uv}_slice = clamp(floor(${i.slice} + 0.5), 0.0, ${o.uv}_cell_count - 1.0);`,
      `vec2 ${o.uv}_cell = vec2(mod(${o.uv}_slice, ${o.uv}_columns), floor(${o.uv}_slice / ${o.uv}_columns));`,
      `vec2 ${o.uv}_local = clamp(${i.uv}, vec2(0.0), vec2(1.0));`,
      `vec2 ${o.uv} = (${o.uv}_local + ${o.uv}_cell) / vec2(${o.uv}_columns, ${o.uv}_rows);`,
      `float ${o.mask} = step(0.0, ${i.uv}.x) * step(${i.uv}.x, 1.0) * step(0.0, ${i.uv}.y) * step(${i.uv}.y, 1.0);`,
      `float ${o.depth} = ${o.uv}_slice / max(${o.uv}_cell_count - 1.0, 1.0);`,
    ].join('\n'),
  },
  StackedSpriteSample: {
    category: 'Fake 3D',
    label: 'Stacked Sprite Sample',
    inputs: [
      { id: 'texture', label: 'Atlas', type: 'image' },
      { id: 'uv', label: 'UV', type: 'vec2', defaultValue: [0.5, 0.5], min: 0, max: 1, step: 0.01 },
      { id: 'columns', label: 'Columns', type: 'float', defaultValue: 4, min: 1, max: 16, step: 1 },
      { id: 'rows', label: 'Rows', type: 'float', defaultValue: 4, min: 1, max: 16, step: 1 },
      { id: 'layers', label: 'Layers', type: 'float', defaultValue: 8, min: 1, max: 16, step: 1 },
      { id: 'view', label: 'View', type: 'vec2', defaultValue: [0.25, -0.18], min: -1, max: 1, step: 0.01 },
      { id: 'depthStrength', label: 'Depth Amt', type: 'float', defaultValue: 0.08, min: 0, max: 0.5, step: 0.001 },
      { id: 'alphaThreshold', label: 'Alpha Cut', type: 'float', defaultValue: 0.01, min: 0, max: 1, step: 0.01 },
    ],
    outputs: [
      { id: 'rgba', label: 'RGBA', type: 'vec4' },
      { id: 'mask', label: 'Mask', type: 'float' },
    ...
    emitGlsl: (i, o) => [
      `float ${o.rgba}_columns = max(floor(${i.columns} + 0.5), 1.0);`,
      `float ${o.rgba}_rows = max(floor(${i.rows} + 0.5), 1.0);`,
      `float ${o.rgba}_cell_count = max(${o.rgba}_columns * ${o.rgba}_rows, 1.0);`,
      `float ${o.rgba}_layers = clamp(floor(${i.layers} + 0.5), 1.0, min(${o.rgba}_cell_count, 16.0));`,
      `vec3 ${o.rgba}_premul = vec3(0.0);`,
      `float ${o.rgba}_alpha = 0.0;`,
      `float ${o.depth} = 0.0;`,
      `for (int ${o.rgba}_i = 0; ${o.rgba}_i < 16; ${o.rgba}_i++) {`,
      `  float ${o.rgba}_layer = float(${o.rgba}_i);`,
      `  float ${o.rgba}_active = 1.0 - step(${o.rgba}_layers, ${o.rgba}_layer);`,
      `  float ${o.rgba}_t = ${o.rgba}_layer / max(${o.rgba}_layers - 1.0, 1.0);`,
      `  vec2 ${o.rgba}_local = ${i.uv} + (${o.rgba}_t - 0.5) * ${i.view} * ${i.depthStrength};`,
      `  float ${o.rgba}_inside = step(0.0, ${o.rgba}_local.x) * step(${o.rgba}_local.x, 1.0) * step(0.0, ${o.rgba}_local.y) * step(${o.rgba}_local.y, 1.0);`,
      `  float ${o.rgba}_slice = clamp(${o.rgba}_layer, 0.0, ${o.rgba}_cell_count - 1.0);`,
      `  vec2 ${o.rgba}_cell = vec2(mod(${o.rgba}_slice, ${o.rgba}_columns), floor(${o.rgba}_slice / ${o.rgba}_columns));`,
      `  vec2 ${o.rgba}_atlas_uv = (clamp(${o.rgba}_local, vec2(0.0), vec2(1.0)) + ${o.rgba}_cell) / vec2(${o.rgba}_columns, ${o.rgba}_rows);`,
      `  vec4 ${o.rgba}_sample = Texel(${i.texture}, ${o.rgba}_atlas_uv);`,
      `  float ${o.rgba}_sample_alpha = ${o.rgba}_sample.a * ${o.rgba}_active * ${o.rgba}_inside;`,
      `  ${o.rgba}_premul += ${o.rgba}_sample.rgb * ${o.rgba}_sample_alpha * (1.0 - ${o.rgba}_alpha);`,
      `  ${o.rgba}_alpha += ${o.rgba}_sample_alpha * (1.0 - ${o.rgba}_alpha);`,
      `  ${o.depth} = max(${o.depth}, ${o.rgba}_t * ${o.rgba}_sample_alpha);`,
      '}',
      `vec4 ${o.rgba} = vec4(${o.rgba}_premul / max(${o.rgba}_alpha, 0.0001), ${o.rgba}_alpha);`,
      `float ${o.mask} = step(${i.alphaThreshold}, ${o.rgba}.a);`,
    ].join('\n'),
  },
  ...
  EffectMix: {
    ...
    label: 'Effect Mix',
    inputs: [
      { id: 'base', label: 'Base', type: 'vec4' },
      { id: 'effect', label: 'Effect', type: 'vec4' },
      { id: 'mask', label: 'Mask', type: 'float', defaultValue: 1, min: 0, max: 1, step: 0.01 },
      { id: 'opacity', label: 'Opacity', type: 'float', defaultValue: 1, min: 0, max: 1, step: 0.01 },
    ],
    outputs: [{ id: 'out', label: 'RGBA', type: 'vec4' }],
    emitGlsl: (i, o) => [
      `float ${o.out}_mix = clamp(${i.mask} * ${i.opacity}, 0.0, 1.0);`,
      `vec4 ${o.out} = mix(${i.base}, ${i.effect}, ${o.out}_mix);`,
    ].join('\n'),
  },
  AlphaMask: {
    category: 'Composite',
    ...
    inputs: [
      { id: 'color', label: 'Color', type: 'vec4' },
      { id: 'threshold', label: 'Threshold', type: 'float', defaultValue: 0.01, min: 0, max: 1, step: 0.01 },
      { id: 'softness', label: 'Soft', type: 'float', defaultValue: 0.01, min: 0, max: 1, step: 0.01 },
      { id: 'invert', label: 'Invert', type: 'float', defaultValue: 0, min: 0, max: 1, step: 1 },
    ],
    outputs: [
      { id: 'mask', label: 'Mask', type: 'float' },
      { id: 'rgba', label: 'RGBA', type: 'vec4' },
    ...
    emitGlsl: (i, o) => [
      `float ${o.mask}_raw = smoothstep(${i.threshold}, ${i.threshold} + max(${i.softness}, 0.0001), ${i.color}.a);`,
      `float ${o.mask}_invert = step(0.5, ${i.invert});`,
      `float ${o.mask} = mix(${o.mask}_raw, 1.0 - ${o.mask}_raw, ${o.mask}_invert);`,
      `vec4 ${o.rgba} = vec4(${i.color}.rgb, ${i.color}.a * ${o.mask});`,
    ].join('\n'),
  },
  LumaMask: {
    ...
    inputs: [
      { id: 'color', label: 'Color', type: 'vec4' },
      { id: 'threshold', label: 'Threshold', type: 'float', defaultValue: 0.5, min: 0, max: 1, step: 0.01 },
      { id: 'softness', label: 'Soft', type: 'float', defaultValue: 0.1, min: 0, max: 1, step: 0.01 },
      { id: 'invert', label: 'Invert', type: 'float', defaultValue: 0, min: 0, max: 1, step: 1 },
    ],
    outputs: [{ id: 'mask', label: 'Mask', type: 'float' }],
    emitGlsl: (i, o) => [
      `float ${o.mask}_luma = dot(${i.color}.rgb, vec3(0.2126, 0.7152, 0.0722));`,
      `float ${o.mask}_raw = smoothstep(${i.threshold}, ${i.threshold} + max(${i.softness}, 0.0001), ${o.mask}_luma);`,
      `float ${o.mask}_invert = step(0.5, ${i.invert});`,
      `float ${o.mask} = mix(${o.mask}_raw, 1.0 - ${o.mask}_raw, ${o.mask}_invert);`,
    ].join('\n'),
  },
  MaskRange: {
    ...
    inputs: [
      { id: 'value', label: 'Value', type: 'float' },
      { id: 'min', label: 'Min', type: 'float', defaultValue: 0, step: 0.01 },
      { id: 'max', label: 'Max', type: 'float', defaultValue: 1, step: 0.01 },
      { id: 'softness', label: 'Soft', type: 'float', defaultValue: 0.01, min: 0, max: 1, step: 0.01 },
      { id: 'invert', label: 'Invert', type: 'float', defaultValue: 0, min: 0, max: 1, step: 1 },
    ],
    outputs: [{ id: 'mask', label: 'Mask', type: 'float' }],
    emitGlsl: (i, o) => [
      `float ${o.mask}_min = min(${i.min}, ${i.max});`,
      `float ${o.mask}_max = max(${i.min}, ${i.max});`,
      `float ${o.mask}_soft = max(${i.softness}, 0.0001);`,
      `float ${o.mask}_raw = smoothstep(${o.mask}_min - ${o.mask}_soft, ${o.mask}_min + ${o.mask}_soft, ${i.value}) * (1.0 - smoothstep(${o.mask}_max - ${o.mask}_soft, ${o.mask}_max + ${o.mask}_soft, ${i.value}));`,
      `float ${o.mask}_invert = step(0.5, ${i.invert});`,
      `float ${o.mask} = mix(${o.mask}_raw, 1.0 - ${o.mask}_raw, ${o.mask}_invert);`,
    ].join('\n'),
  },
  ColorKeyMask: {
    category: 'Composite',
    label: 'Color Key Mask',
    inputs: [
      { id: 'source', label: 'Source', type: 'vec4' },
      { id: 'targetColor', label: 'Target', type: 'vec4', defaultValue: [1, 0, 0, 1], min: 0, max: 1, step: 0.01 },
      { id: 'tolerance', label: 'Tolerance', type: 'float', defaultValue: 0.12, min: 0, max: 1, step: 0.01 },
      { id: 'softness', label: 'Soft', type: 'float', defaultValue: 0.02, min: 0, max: 1, step: 0.01 },
      { id: 'invert', label: 'Invert', type: 'float', defaultValue: 0, min: 0, max: 1, step: 1 },
    ],
    outputs: [
      { id: 'mask', label: 'Mask', type: 'float' },
    ...
    emitGlsl: (i, o) => [
      `float ${o.mask}_distance = distance(${i.source}.rgb, ${i.targetColor}.rgb);`,
      `float ${o.mask}_tolerance = max(${i.tolerance}, 0.0);`,
      `float ${o.mask}_soft = max(${i.softness}, 0.0001);`,
      `float ${o.mask}_raw = 1.0 - smoothstep(${o.mask}_tolerance, ${o.mask}_tolerance + ${o.mask}_soft, ${o.mask}_distance);`,
      `float ${o.mask}_invert = step(0.5, ${i.invert});`,
      `float ${o.mask} = mix(${o.mask}_raw, 1.0 - ${o.mask}_raw, ${o.mask}_invert);`,
      `vec4 ${o.rgba} = vec4(${i.source}.rgb * ${o.mask}, ${i.source}.a * ${o.mask});`,
    ].join('\n'),
  },
  PaletteSwap: {
    category: 'Composite',
    ...
    inputs: [
      { id: 'source', label: 'Source', type: 'vec4' },
      { id: 'fromA', label: 'From A', type: 'vec4', defaultValue: [1, 0, 0, 1], min: 0, max: 1, step: 0.01 },
      { id: 'toA', label: 'To A', type: 'vec4', defaultValue: [0, 1, 0, 1], min: 0, max: 1, step: 0.01 },
      { id: 'fromB', label: 'From B', type: 'vec4', defaultValue: [0, 1, 0, 1], min: 0, max: 1, step: 0.01 },
      { id: 'toB', label: 'To B', type: 'vec4', defaultValue: [0, 0, 1, 1], min: 0, max: 1, step: 0.01 },
      { id: 'fromC', label: 'From C', type: 'vec4', defaultValue: [0, 0, 1, 1], min: 0, max: 1, step: 0.01 },
      { id: 'toC', label: 'To C', type: 'vec4', defaultValue: [1, 0, 0, 1], min: 0, max: 1, step: 0.01 },
      { id: 'pairs', label: 'Pairs', type: 'float', defaultValue: 1, min: 1, max: 3, step: 1 },
      { id: 'tolerance', label: 'Tolerance', type: 'float', defaultValue: 0.08, min: 0, max: 1, step: 0.01 },
      { id: 'softness', label: 'Soft', type: 'float', defaultValue: 0.02, min: 0, max: 1, step: 0.01 },
      { id: 'strength', label: 'Strength', type: 'float', defaultValue: 1, min: 0, max: 1, step: 0.01 },
    ],
    outputs: [
      { id: 'rgba', label: 'RGBA', type: 'vec4' },
      { id: 'mask', label: 'Mask', type: 'float' },
    ...
    emitGlsl: (i, o) => [
      `float ${o.mask}_tolerance = max(${i.tolerance}, 0.0);`,
      `float ${o.mask}_soft = max(${i.softness}, 0.0001);`,
      `float ${o.mask}_pairs = clamp(floor(${i.pairs} + 0.5), 1.0, 3.0);`,
      `float ${o.mask}_a = 1.0 - smoothstep(${o.mask}_tolerance, ${o.mask}_tolerance + ${o.mask}_soft, distance(${i.source}.rgb, ${i.fromA}.rgb));`,
      `float ${o.mask}_b = (1.0 - smoothstep(${o.mask}_tolerance, ${o.mask}_tolerance + ${o.mask}_soft, distance(${i.source}.rgb, ${i.fromB}.rgb))) * step(1.5, ${o.mask}_pairs);`,
      `float ${o.mask}_c = (1.0 - smoothstep(${o.mask}_tolerance, ${o.mask}_tolerance + ${o.mask}_soft, distance(${i.source}.rgb, ${i.fromC}.rgb))) * step(2.5, ${o.mask}_pairs);`,
      `float ${o.mask}_sum = ${o.mask}_a + ${o.mask}_b + ${o.mask}_c;`,
      `float ${o.mask} = clamp(${o.mask}_sum, 0.0, 1.0) * clamp(${i.source}.a, 0.0, 1.0);`,
      `vec4 ${o.rgba}_target = (${i.toA} * ${o.mask}_a + ${i.toB} * ${o.mask}_b + ${i.toC} * ${o.mask}_c) / max(${o.mask}_sum, 0.0001);`,
      `${o.rgba}_target.a *= ${i.source}.a;`,
      `vec4 ${o.rgba} = mix(${i.source}, ${o.rgba}_target, clamp(${o.mask} * ${i.strength}, 0.0, 1.0));`,
    ].join('\n'),
  },
  GradientMap: {
    category: 'Composite',
    ...
    inputs: [
      { id: 'value', label: 'Value', type: 'float' },
      { id: 'colorA', label: 'Color A', type: 'vec4', defaultValue: [0, 0, 0, 1], min: 0, max: 1, step: 0.01 },
      { id: 'colorB', label: 'Color B', type: 'vec4', defaultValue: [1, 1, 1, 1], min: 0, max: 1, step: 0.01 },
    ],
    outputs: [{ id: 'rgba', label: 'RGBA', type: 'vec4' }],
    emitGlsl: (i, o) => `vec4 ${o.rgba} = mix(${i.colorA}, ${i.colorB}, clamp(${i.value}, 0.0, 1.0));`,
  },
  MaskCombine: {
    category: 'Composite',
    label: 'Mask Combine',
    inputs: [
      { id: 'a', label: 'A', type: 'float', defaultValue: 1, min: 0, max: 1, step: 0.01 },
      { id: 'b', label: 'B', type: 'float', defaultValue: 1, min: 0, max: 1, step: 0.01 },
    ],
    outputs: [
      { id: 'multiply', label: 'Multiply', type: 'float' },
      { id: 'add', label: 'Add', type: 'float' },
      { id: 'max', label: 'Max', type: 'float' },
      { id: 'min', label: 'Min', type: 'float' },
      { id: 'subtract', label: 'Subtract', type: 'float' },
    ],
    emitGlsl: (i, o) => [
      `float ${o.multiply}_a = clamp(${i.a}, 0.0, 1.0);`,
      `float ${o.multiply}_b = clamp(${i.b}, 0.0, 1.0);`,
      `float ${o.multiply} = ${o.multiply}_a * ${o.multiply}_b;`,
      `float ${o.add} = clamp(${o.multiply}_a + ${o.multiply}_b, 0.0, 1.0);`,
      `float ${o.max} = max(${o.multiply}_a, ${o.multiply}_b);`,
      `float ${o.min} = min(${o.multiply}_a, ${o.multiply}_b);`,
      `float ${o.subtract} = clamp(${o.multiply}_a - ${o.multiply}_b, 0.0, 1.0);`,
    ].join('\n'),
  },
  BlendModes: {
    category: 'Composite',
    ...
    inputs: [
      { id: 'base', label: 'Base', type: 'vec4' },
      { id: 'layer', label: 'Layer', type: 'vec4' },
      { id: 'mask', label: 'Mask', type: 'float', defaultValue: 1, min: 0, max: 1, step: 0.01 },
      { id: 'opacity', label: 'Opacity', type: 'float', defaultValue: 1, min: 0, max: 1, step: 0.01 },
    ],
    outputs: [
      { id: 'normal', label: 'Normal', type: 'vec4' },
      { id: 'multiply', label: 'Multiply', type: 'vec4' },
      { id: 'screen', label: 'Screen', type: 'vec4' },
      { id: 'overlay', label: 'Overlay', type: 'vec4' },
      { id: 'add', label: 'Add', type: 'vec4' },
      { id: 'difference', label: 'Difference', type: 'vec4' },
    ],
    emitGlsl: (i, o) => [
      `vec4 ${o.normal}_base = vec4(clamp(${i.base}.rgb, 0.0, 1.0), clamp(${i.base}.a, 0.0, 1.0));`,
      `vec4 ${o.normal}_layer = vec4(clamp(${i.layer}.rgb, 0.0, 1.0), clamp(${i.layer}.a, 0.0, 1.0));`,
      `float ${o.normal}_mix = clamp(${i.mask} * ${i.opacity}, 0.0, 1.0) * ${o.normal}_layer.a;`,
      `float ${o.normal}_alpha = mix(${o.normal}_base.a, max(${o.normal}_base.a, ${o.normal}_layer.a), ${o.normal}_mix);`,
      `vec3 ${o.multiply}_rgb = ${o.normal}_base.rgb * ${o.normal}_layer.rgb;`,
      `vec3 ${o.screen}_rgb = 1.0 - (1.0 - ${o.normal}_base.rgb) * (1.0 - ${o.normal}_layer.rgb);`,
      `vec3 ${o.overlay}_dark = 2.0 * ${o.normal}_base.rgb * ${o.normal}_layer.rgb;`,
      `vec3 ${o.overlay}_light = 1.0 - 2.0 * (1.0 - ${o.normal}_base.rgb) * (1.0 - ${o.normal}_layer.rgb);`,
      `vec3 ${o.overlay}_rgb = mix(${o.overlay}_dark, ${o.overlay}_light, step(vec3(0.5), ${o.normal}_base.rgb));`,
      `vec3 ${o.add}_rgb = clamp(${o.normal}_base.rgb + ${o.normal}_layer.rgb, 0.0, 1.0);`,
      `vec3 ${o.difference}_rgb = abs(${o.normal}_base.rgb - ${o.normal}_layer.rgb);`,
      `vec4 ${o.normal} = vec4(mix(${o.normal}_base.rgb, ${o.normal}_layer.rgb, ${o.normal}_mix), ${o.normal}_alpha);`,
      `vec4 ${o.multiply} = vec4(mix(${o.normal}_base.rgb, ${o.multiply}_rgb, ${o.normal}_mix), ${o.normal}_alpha);`,
      `vec4 ${o.screen} = vec4(mix(${o.normal}_base.rgb, ${o.screen}_rgb, ${o.normal}_mix), ${o.normal}_alpha);`,
      `vec4 ${o.overlay} = vec4(mix(${o.normal}_base.rgb, ${o.overlay}_rgb, ${o.normal}_mix), ${o.normal}_alpha);`,
      `vec4 ${o.add} = vec4(mix(${o.normal}_base.rgb, ${o.add}_rgb, ${o.normal}_mix), ${o.normal}_alpha);`,
      `vec4 ${o.difference} = vec4(mix(${o.normal}_base.rgb, ${o.difference}_rgb, ${o.normal}_mix), ${o.normal}_alpha);`,
    ].join('\n'),
  },
  ColorRamp: {
    category: 'Composite',
    ...
    inputs: [
      { id: 'value', label: 'Value', type: 'float' },
      { id: 'low', label: 'Low', type: 'vec4', defaultValue: [0, 0, 0, 1], min: 0, max: 1, step: 0.01 },
      { id: 'mid', label: 'Mid', type: 'vec4', defaultValue: [0.5, 0.5, 0.5, 1], min: 0, max: 1, step: 0.01 },
      { id: 'high', label: 'High', type: 'vec4', defaultValue: [1, 1, 1, 1], min: 0, max: 1, step: 0.01 },
      { id: 'midpoint', label: 'Midpoint', type: 'float', defaultValue: 0.5, min: 0, max: 1, step: 0.01 },
    ],
    outputs: [{ id: 'rgba', label: 'RGBA', type: 'vec4' }],
    emitGlsl: (i, o) => [
      `float ${o.rgba}_value = clamp(${i.value}, 0.0, 1.0);`,
      `float ${o.rgba}_midpoint = clamp(${i.midpoint}, 0.0001, 0.9999);`,
      `vec4 ${o.rgba}_low_mid = mix(${i.low}, ${i.mid}, clamp(${o.rgba}_value / ${o.rgba}_midpoint, 0.0, 1.0));`,
      `vec4 ${o.rgba}_mid_high = mix(${i.mid}, ${i.high}, clamp((${o.rgba}_value - ${o.rgba}_midpoint) / (1.0 - ${o.rgba}_midpoint), 0.0, 1.0));`,
      `vec4 ${o.rgba} = mix(${o.rgba}_low_mid, ${o.rgba}_mid_high, step(${o.rgba}_midpoint, ${o.rgba}_value));`,
    ].join('\n'),
  },
  Brightness: {
    category: 'Color',
...
export const CATEGORY_ORDER: Array<{ category: NodeCategory; nodes: NodeType[] }> = [
  ...
  { category: 'Composite', nodes: ['CompositeAlpha', 'EffectMix', 'AlphaMask', 'LumaMask', 'MaskRange', 'ColorKeyMask', 'PaletteSwap', 'GradientMap', 'MaskCombine', 'BlendModes', 'ColorRamp'] },
  ...
  { category: 'Fake 3D', nodes: ['SpriteTextureSample', 'BillboardUV', 'ParallaxUV', 'FakeDepthShade', 'BillboardShadow', 'AtlasSliceUV', 'StackedSpriteSample'] },
  {
    category: 'Effect',
    nodes: [
      'SampleTexture',
