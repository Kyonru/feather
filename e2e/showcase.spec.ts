...
test('texture lab generates textures and feeds creative tools in the showcase', async ({ page }) => {
  ...
  await page.getByTestId('texture-lab-shape-move-handle').scrollIntoViewIfNeeded();
  const rectMoveHandle = await page.getByTestId('texture-lab-shape-move-handle').boundingBox();
  expect(rectMoveHandle).not.toBeNull();
  await page.mouse.move(rectMoveHandle!.x + rectMoveHandle!.width / 2, rectMoveHandle!.y + rectMoveHandle!.height / 2);
  await page.mouse.down();
  await page.mouse.move(rectMoveHandle!.x + rectMoveHandle!.width / 2 + 36, rectMoveHandle!.y + rectMoveHandle!.height / 2 + 24, {
    steps: 8,
  });
  await page.mouse.up();
  await expect.poll(() => preview.getAttribute('src')).not.toBe(withRect);
  const resizedRect = await preview.getAttribute('src');
  await page.getByTitle('Disable Rect').click();
  await expect.poll(() => preview.getAttribute('src')).not.toBe(resizedRect);
...
test('particle playground timeline toggles emitters and moves grouped items in the showcase', async ({ page }) => {
  ...
  await page.getByTestId('particle-timeline-track-1').getByRole('button', { name: 'Disable emitter Flame' }).click();
  await expect(page.getByTestId('particle-timeline-track-1').getByText('muted')).toBeVisible();
  await page.getByTestId('particle-timeline-track-1').getByRole('button', { name: 'Enable emitter Flame' }).click();
...
test('shader graph preview probes render texture-heavy uploads in the showcase', async ({ page }) => {
  await page.goto('/shader-graph');
  await expect(page.getByRole('heading', { name: 'Shader Graph' })).toBeVisible();
